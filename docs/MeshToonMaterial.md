...
## Constructor
`newMeshToonMaterial( parameters :Object)`
...
- `.gradientMap :Texture` — Gradient map for toon shading. It's required to set Texture#minFilter and Texture#magFilter to {@linkNearestFilter}
when using this type of texture. Default is null .
- `.isMeshToonMaterial : boolean` — This flag can be used for type testing. Default is true .
