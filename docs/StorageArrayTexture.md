...
## Constructor
`newStorageArrayTexture( width :number, height :number, depth :number)`
...
## Methods
- `.setSize( width :number, height :number, depth :number)` — Sets the size of the storage array texture.
