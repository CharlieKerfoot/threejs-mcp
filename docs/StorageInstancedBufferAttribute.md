...
Extends: BufferAttribute→InstancedBufferAttribute→
...
## Constructor
`newStorageInstancedBufferAttribute( count :number | TypedArray, itemSize :number, typeClass :TypedArray.constructor)`
Constructs a new storage instanced buffer attribute.

## Properties
- `.isStorageInstancedBufferAttribute : boolean` — This flag can be used for type testing. Default is true .
